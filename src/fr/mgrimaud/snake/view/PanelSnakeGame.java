...
import fr.mgrimaud.snake.utils.*;
...
public class PanelSnakeGame extends JPanel{
	...
	private ArrayList<FeaturesSnake> featuresSnakes = new ArrayList<FeaturesSnake>();
	private ArrayList<FeaturesItem> featuresItems = new ArrayList<FeaturesItem>();
		...
		for(int x=0; x<sizeX; x++)
		{
			...
			for(int y=0; y<sizeY; y++)
			{
				if (walls[x][y]){
					...
					try {
						Image img = ImageIO.read(new File("img/wall.png"));
		...
		for(int i = 0; i < positions.size(); i++) {
			...
			try {
				if(featuresSnake.getColorSnake() == ColorSnake.Green) {
					img = ImageIO.read(new File("img/snake_green_" + cpt_img + ".png"));
				} else if(featuresSnake.getColorSnake() == ColorSnake.Red ) {
					img = ImageIO.read(new File("img/snake_red_" + cpt_img + ".png"));
				}
	...
	void paint_Item(Graphics g, FeaturesItem featuresItem){
		...
		if (featuresItem.getItemType() == ItemType.APPLE) {
			try {
				Image img = ImageIO.read(new File("img/apple.png"));
				g.drawImage(img, (int)pos_x, (int)pos_y, (int)stepx, (int)stepy, this);
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (featuresItem.getItemType() == ItemType.BOX) {
			try {
				Image img = ImageIO.read(new File("img/mysteryBox.png"));
				g.drawImage(img, (int)pos_x, (int)pos_y, (int)stepx, (int)stepy, this);
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (featuresItem.getItemType() == ItemType.SICK_BALL) {
			try {
				Image img = ImageIO.read(new File("img/sickBall.png"));
				g.drawImage(img, (int)pos_x, (int)pos_y, (int)stepx, (int)stepy, this);
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (featuresItem.getItemType() == ItemType.INVINCIBILITY_BALL) {
			try {
				Image img = ImageIO.read(new File("img/invicibleBall.png"));
				g.drawImage(img, (int)pos_x, (int)pos_y, (int)stepx, (int)stepy, this);
			} catch (IOException e) {
				e.printStackTrace();
			}
